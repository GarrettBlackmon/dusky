...
if [[ -n "$selection" ]]; then
    ...
    if [[ -n "$full_path" && -f "$full_path" ]]; then
        ...
        # Apply wallpaper transition synchronously (wait for completion)
        swww img "$full_path" \
            --transition-type grow \
            --transition-duration 2 \
            --transition-fps 60

        # Then generate theme after wallpaper is fully visible
        # Run in background but wait for completion before script exits
